...
contract Voting {
  ...
  constructor() {
    ...
    function vote(uint _idVoting, address _elected) public payable {
      ...
      voting[_idVoting].voters[_elected].amountVote++;
      voting[_idVoting].numberVoters++;

      if(voting[_idVoting].voters[_elected].amountVote > voting[_idVoting].countVote){
        voting[_idVoting].countVote = voting[_idVoting].voters[_elected].amountVote;
